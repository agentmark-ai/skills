...
For local execution, ensure `agentmark dev` is running in another shell. `run-prompt` posts to the **webhook server** (default `http://localhost:9417`), not the API server on 9418. Override with `--server <url>` or `AGENTMARK_WEBHOOK_URL`.
...
- **Inventing a `metadata:` / `model:` frontmatter shape** (e.g. `metadata: { model: { name: ... } }`, a convention from other frameworks). AgentMark has no `metadata` model block — the prompt kind is determined solely by which `*_config` block is present, so this fails with "Could not determine prompt kind." Use a `text_config` block with the model under its `model_name` key, as shown above.
...
- **Hard-coding model IDs that don't exist** — run `agentmark pull-models` for a current list, or fetch `https://docs.agentmark.co/getting-started/client-setup.md`. Real examples from the docs include `gpt-5-mini`, `gpt-5`, `claude-4-sonnet-20250514`. Do not invent versions.

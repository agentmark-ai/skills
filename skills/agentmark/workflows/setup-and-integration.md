...
3. `running prompts` (`/build/running-prompts`) for in-process integration (Path A — the usual case), or `connect your SDK` (`/getting-started/client-setup#connect-your-sdk`) for the Cloud-executor path (Path B)
...
**First, on a greenfield project with no `package.json` / `pyproject.toml`:** `agentmark init` deliberately doesn't create one — it can't know your stack — so the manifest is yours to add here, *before* the SDK files that import into it. Initialize a minimal one for the detected language (`npm init -y` for TypeScript; a `pyproject.toml` or `requirements.txt` for Python), then declare the runtime deps the docs page named (`@agentmark-ai/prompt-core`, `@agentmark-ai/sdk`, your provider SDK). The scaffolded client and the `dev-entry.ts` / `handler.ts` you're about to write import those packages, so without a manifest there is nothing to install into and the Step 6 smoke can't boot. (An *existing* repo already has its manifest — add the deps to it; don't create a second one.)

**`agentmark init` already scaffolds the client** (`agentmark.client.ts` / `agentmark_client.py`) — the provider-agnostic loader + scorers file. **Keep it; do not rewrite it from scratch.** Read it, confirm it matches the docs, and only edit it to register real scorers. What you DO still author are the two SDK-specific files: the `dev-entry.ts` dev-server entry and the `handler.ts` deployment entry, whose executor wraps *your* SDK call. For their full recipe, fetch `set up your AgentMark client` (Getting Started → Client setup) from the docs MCP and follow it verbatim, including its verification step (`agentmark dev` + `run-prompt`). The dashboard's Run buttons stay disabled until the client is deployed (or a webhook is set), so a Cloud-mode setup isn't complete without it.
...
Verify the scaffold before handing back. **First install the project's dependencies** (`npm install` or the detected package manager for TypeScript; `pip install` / `uv sync` for Python) as soon as `package.json` / `pyproject.toml` exists — `agentmark dev` and `doctor --smoke --boot` boot the dev server, which exits with code 1 when `node_modules` / the venv is missing (the most common smoke-boot failure). Then run `agentmark doctor` first: it statically checks `agentmark.json`, the client / dev-entry / handler files, prompts + `builtInModels`, and deps, and prints a concrete fix for anything wrong. Then boot the dev server and run an end-to-end check with `agentmark doctor --smoke`: it runs the prompt and confirms the emitted trace round-trips with the right shape (a model, token usage, input, output), which is the fastest way to catch a bad key, an SDK mismatch, or unwired tracing. Exact commands and flags via `agentmark <cmd> --help`. **Do not encode CLI surface in this workflow.** **`agentmark doctor --smoke` must PASS, not merely run, before you hand back** — if it fails (commonly uninstalled deps or a missing `dev-entry.ts`), fix the named cause and re-run; never report setup complete on a red smoke.
...
- **Hunting for an "adapter," or defaulting to the custom-adapter page.** There are no SDK-specific adapters — every SDK integrates through the neutral render. Route to Path A (lightweight, no executor — [`/build/running-prompts`](https://docs.agentmark.co/build/running-prompts.md)) or Path B (`createExecutor`, cloud-executed — [`/getting-started/client-setup#connect-your-sdk`](https://docs.agentmark.co/getting-started/client-setup.md)). A full custom adapter only earns its keep when you want AgentMark to own the provider request shape — rarely what these users need.
